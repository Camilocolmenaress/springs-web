...
function sv(zones: ReturnType<typeof useDesignConfig>["config"]["zones"], zone: string, element: string, prop: string, fallback: number): number {
...
export default function MobileHome() {
  ...
  const { config, editMode, saved, updateProp, save, reset, exportValues } = useDesignConfig("home-mobile");
  const z = config.zones;

  // ── Leer valores del config ──────────────────────────────────
  const d = {
    // hero
  ...
  const spring = { stiffness: d.scrollStiffness, damping: d.scrollDamping };
  const bagX   = useSpring(bagXRaw,  spring);
  const bagOp  = useSpring(bagOpRaw, spring);
  const boxX   = useSpring(boxXRaw,  spring);
  const boxOp  = useSpring(boxOpRaw, spring);
  const cupX   = useSpring(cupXRaw,  spring);
  const cupOp  = useSpring(cupOpRaw, spring);
  ...
  return (
    <div style={{ display: "flex", height: "100dvh" }}>
      ...
      {/* ── Editor sidebar ─────────────────────────────────── */}
      {editMode && (
        <div style={{ width: 320, flexShrink: 0, height: "100dvh", overflow: "hidden" }}>
          <DevPanel
            config={config}
            saved={saved}
            onUpdate={updateProp}
      ...
      <div
        ...
        style={{ flex: 1, background: C.cream, height: "100dvh", overflowY: "auto", overflowX: "hidden" }}
      >
        ...
        <section style={{ background: C.cream, position: "relative" }}>
          ...
          <motion.div
            ...
            style={{
              margin: `${d.sensMarginV}px ${d.sensMarginH}px`,
        ...
        >
          <div style={{
            ...
            ...F.display, fontSize: "45vw", color: C.tinta, opacity: 0.03,
        ...
        <section style={{ background: C.burgundy, padding: "56px 18px 72px 18px", position: "relative", overflow: "hidden" }}>
          <div style={{
            ...
            ...F.display, fontSize: "22vw", color: C.cream, opacity: 0.04,
            letterSpacing: "-0.02em", whiteSpace: "nowrap",
            zIndex: 0, pointerEvents: "none", userSelect: "none",
            transform: "rotate(-12deg)", transformOrigin: "right bottom",
          }}>
            SPRINGS
          </div>
          ...
          <div style={{ position: "relative", zIndex: 1 }}>
            ...
            <FadeUp delay={0.16}>
              <div style={{ display: "flex", flexDirection: "column", gap: d.appsGap, marginBottom: d.appsMb }}>
                <a href="#" style={{ ...F.display, fontSize: "1rem", letterSpacing: "0.12em", color: C.tinta, background: C.cream, padding: `${d.appsPaddingV}px 24px`, textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  RAPPI <span>→</span>
                </a>
                <a href="#" style={{ ...F.display, fontSize: "1rem", letterSpacing: "0.12em", color: C.cream, background: "transparent", border: `1px solid ${C.cream}`, opacity: 0.9, padding: `${d.appsPaddingV}px 24px`, textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  UBER EATS <span>→</span>
                </a>
                <a href="/menu" style={{ ...F.display, fontSize: "1rem", letterSpacing: "0.12em", color: C.mostaza, background: "transparent", border: `1px solid ${C.mostaza}`, padding: `${d.appsPaddingV}px 24px`, textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
